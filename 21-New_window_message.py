    ...
    **Steps Explained:**
    ...
    with sync_playwright() as playwright:
        ...
        with context.expect_page() as new_page_info:
            # page.click("#tabButton")  # Button that opens a new tab
            page.click("#messageWindowButton")  # Button that opens a new tab

        # Get the new tab reference
        new_page = new_page_info.value
        new_page.wait_for_load_state()  # Wait for the new tab to fully load
        time.sleep(2)  # Adjust this time if needed based on alert appearance
